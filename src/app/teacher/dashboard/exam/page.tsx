      ...
      cell: ({ row }) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => router.push(`/teacher/dashboard/exam/${row.original.id}`)}>
              <Eye className="mr-2 h-4 w-4" /> View Details
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => router.push(`/teacher/dashboard/exam/edit/${row.original.id}`)}>
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Exams" description="Create and manage examinations" breadcrumbs={[{ label: "Exams" }]}
        actions={
          <Button asChild>
            <Link href="/teacher/dashboard/exam/create">
