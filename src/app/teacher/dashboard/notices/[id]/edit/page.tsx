...
export default function EditNoticePage({
  ...
  const getNoticeById = useNoticeStore((state) => state.getNoticeById);
  ...
  const onSubmit = async (data: NoticeFormData) => {
    try {
      ...
      router.push(`/teacher/dashboard/notices/${id}`);
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumbs={[
          { label: "Notices", href: "/teacher/dashboard/notices" },
          { label: notice.title, href: `/teacher/dashboard/notices/${id}` },
          { label: "Edit" },
        ]}
        actions={
          <Button variant="outline" asChild id="view-notice-btn">
            <Link href={`/teacher/dashboard/notices/${id}`}>
